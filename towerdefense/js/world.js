...
function renderScale(w,h){
  // small embeds get a sharper buffer, huge ones a cheaper one
...
W.init = function(el){
  ...
  sun.castShadow = true;
  sun.shadow.mapSize.set(navigator.maxTouchPoints>0 ? 1024 : 2048, navigator.maxTouchPoints>0 ? 1024 : 2048);
...
W.frame = function(){
  ...
  // the build bar eats the bottom of the screen: push the island up a little
  const shift = Math.round(vh * (portrait ? -0.02 : 0.055));   // portrait has spare room below the island
...
W.toScreen = function(v){
  const q = v.clone().project(camera); const [vw,vh] = viewportSize();
  return [ (q.x+1)/2*vw, (1-q.y)/2*vh ];
};
...
W.buildMap = function(map){
  ...
  hemi.color.set(map.theme==='snow'?0xeaf4ff:0xdfefff);
  sun.position.set(map.w/2 - 6, 14, map.h/2 + 5); sun.target.position.set(map.w/2, 0, map.h/2);

  // path cells (the route is axis aligned between waypoints)
  ...
  map.pathSet = pathSet;
  // world-space route points (cell centres); the first and last are pushed off the island
  ...
  // wheel ruts / pebbles on the road
  ...
  cliff.position.set(map.w/2, -0.72, map.h/2); mapGroup.add(cliff);   // top face just under the ground plane, no z-fight
  ...
  // props on the rim only (never on buildable cells or the road openings)
  ...
  const treeMat = mat(theme.tree), trunkMat = mat(theme.trunk), rockMat = mat(theme.rock), snowMat = mat(0xf4f8fc);
  ...
  // castle at the end of the road, dark gate at the start
  ...
  gate.lookAt(route[1].clone().setY(0.6)); mapGroup.add(gate); map.gate = gate;
...
function makeGate(){
  // a glowing portal in the ground with a stone arch over it — reads from straight above too
  const g = new T.Group();
...
W.makeTower = function(type, level){
  const d = TD.TOWERS[type], g = new T.Group();
  const stone = mat(0xa9a59b), stoneD = mat(0x7f7b72), wood = mat(0x8a5a2b), gold = emissive(0xffc857, 0.35);
  ...
  const body = mat(d.color), acc = emissive(d.accent, 0.5);
  if(type==='archer'){
    ...
    const bow = new T.Mesh(new T.TorusGeometry(0.24, 0.03, 6, 14, Math.PI), mat(0x5a3a1a)); bow.rotation.set(-Math.PI/2, 0, 0); bow.position.z = 0.12; head.add(bow);
    ...
    const drum = new T.Mesh(new T.SphereGeometry(0.19, 10, 8), mat(0x3f424b)); head.add(drum);
    ...
    const crystal = new T.Mesh(new T.OctahedronGeometry(0.26, 0), emissive(d.color, 0.7)); crystal.castShadow = true; head.add(crystal); g.spin = crystal;
    ...
    const coil = new T.Mesh(new T.CylinderGeometry(0.12,0.16,0.6,8), mat(0x7a6cc2)); coil.position.y = 0.58; coil.castShadow = true; g.add(coil);
    for(let i=0;i<3;i++){ const r = new T.Mesh(new T.TorusGeometry(0.19,0.025,6,14), mat(0xc9b8ff)); r.rotation.x = Math.PI/2; r.position.y = 0.4+i*0.16; g.add(r); }
    head.position.y = 1.0;
    const orb = new T.Mesh(new T.SphereGeometry(0.2, 12, 10), emissive(d.accent, 0.9)); head.add(orb); g.spin = orb;
    g.muzzle.set(0,0,0);
  }else if(type==='sniper'){
    const tower = new T.Mesh(new T.CylinderGeometry(0.15,0.22,0.9,8), stoneD); tower.position.y = 0.73; tower.castShadow = true; g.add(tower);
    head.position.y = 1.25;
    const rail = new T.Mesh(new T.CylinderGeometry(0.05,0.06,0.9,8), body); rail.rotation.x = Math.PI/2; rail.position.z = 0.3; rail.castShadow = true; head.add(rail);
  ...
  // upgrade trim: gold rings and a banner
  ...
  if(level>=3){
    ...
    const flag = new T.Mesh(new T.PlaneGeometry(0.26,0.16), new T.MeshLambertMaterial({color:d.accent, side:T.DoubleSide})); flag.position.set(-0.23,0.76,-0.2); g.add(flag);
  }
  return g;
};
...
W.makeEnemy = function(type){
  ...
  const body = mat(d.color), dark = mat(0x2a2430), skin = mat(0xf1c9a0);
  const torso = new T.Mesh(new T.SphereGeometry(s, 12, 10), body); torso.scale.set(1,1.15,0.9); torso.position.y = s*1.35; torso.castShadow = true; g.add(torso);
  const head = new T.Mesh(new T.SphereGeometry(s*0.62, 10, 8), type==='knight'?mat(0x6f7a8a):skin); head.position.y = s*2.65; head.castShadow = true; g.add(head);
  for(const sx of [-1,1]){ const eye = new T.Mesh(new T.SphereGeometry(s*0.1, 6, 5), mat(0x1a1620)); eye.position.set(sx*s*0.24, s*2.72, s*0.52); g.add(eye); }
  const legs = [];
  ...
  g.legs = legs; g.torso = torso; g.head = head; g.baseY = 0;
  ...
  if(type==='boss'){
    ...
    torso.material = emissive(d.color, 0.25);
  }
  // health bar: two thin boxes that always face the camera; the fill is anchored on the left
  const bar = new T.Group(); bar.position.y = s*3.5 + (type==='boss'?0.3:0.1);
...
// ---- projectiles ----
const projPool = { arrow:[], shell:[], ice:[] };
...
// ---- range ring + build ghost ----
let ring = null, ghost = null;
...
function fxTick(dt){
  ...
  // compact occasionally so dead slots get reused from the top
